...
{
    uint16_t touch_x[5] = {0};
    uint16_t touch_y[5] = {0};
    uint8_t touch_cnt = 0;

    while (1) {
        esp_lcd_touch_read_data(touch_handle);                    // 必须先读取原始数据

        if (esp_lcd_touch_get_coordinates(touch_handle, touch_x, touch_y, NULL, &touch_cnt, 5)) {
            if (touch_cnt > 0) {
                ESP_LOGI(TAG, "Touch detected! Points: %d", touch_cnt);
                for (uint8_t i = 0; i < touch_cnt; i++) {
                    ESP_LOGI(TAG, "  Point %d → X=%d, Y=%d", i, touch_x[i], touch_y[i]);
                }
            }
        }

        vTaskDelay(pdMS_TO_TICKS(30));   // 约 33Hz，够用且不占 CPU
    }
}
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "System started. Touch coordinates will be printed to serial port.");
}